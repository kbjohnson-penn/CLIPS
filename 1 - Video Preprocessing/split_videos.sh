...
for file in *.{mp4,mov,avi,mkv}; do
    ...
    for ((i=0; i<parts; i++)); do
        start=$((i * 60))
        ffmpeg -i "$file" -ss "$start" -t 60 -c:v "$codec_video" -b:v "$video_bitrate" -r "$framerate" -s "$resolution" -c:a "$codec_audio" -b:a "$audio_bitrate" -preset "$preset" "${file%.*}_Part_$((i+1)).mp4"
    done

    # Handle last segment if it needs to be appended
    if [ "$append_last_part" = true ]; then
        start=$((parts * 60))
        ffmpeg -i "$file" -ss "$start" -c:v "$codec_video" -b:v "$video_bitrate" -r "$framerate" -s "$resolution" -c:a "$codec_audio" -b:a "$audio_bitrate" -preset "$preset" "${file%.*}_Part_$((parts + 1)).mp4"
    elif [ "$last_part" -ge 30 ]; then
        start=$((parts * 60))
        ffmpeg -i "$file" -ss "$start" -t "$last_part" -c:v "$codec_video" -b:v "$video_bitrate" -r "$framerate" -s "$resolution" -c:a "$codec_audio" -b:a "$audio_bitrate" -preset "$preset" "${file%.*}_Part_$((parts + 1)).mp4"
    fi
